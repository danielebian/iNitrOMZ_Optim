function Data = bgc1d_process_rates(Data1, bgc)
 ...
 if isnan(depthox(1))
    ...
    for v=1:length(Data.rates.name)
       Data.rates.val(v,:) = bgc1d_griddata(Data.rates.(Data.rates.name{v}),Data.rates.depth_from_oxicline+depthox(1), bgc) .* ...
                             Data.rates.convf(v); % convert from nM-N day^{-1} to mmol m^{-3} s^{-1}
    end
 end
